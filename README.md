...
> **桌面软件 / 授权**：已内置离线「一机一码」授权与桌面启动器（`launcher\地理探测器.vbs`，无控制台、应用窗口模式），可用 Inno Setup（`installer\setup.iss`）打包成 Windows 安装包。授权原理、激活码发放、打包步骤见 **[授权与打包说明.md](授权与打包说明.md)**。首次启动会显示激活页，需用 `tools\keygen.R` 为机器码签发激活码。
...
| 页面 | 功能 |
|---|---|
| ① 数据上传 | CSV / Excel 多文件上传（自动识别 UTF-8 / GBK 编码，`<空>` 自动转 NA）；解析带**加载动画**；预览显示原始表；**面板数据自动解析模块**：自动分离动态变量（带年份🔄）/ 静态变量（常量📌），过滤 OBJECTID/Shape/ORIG_FID/FID/Id 等字段（含 `OBJECTID *`/`Shape *` 带尾缀的写法）——① 年份按钮多选（默认全选）② Y 下拉单选 ③ X 标签按点击顺序命名 x1,x2…，选中后点「连续/分类」标签切换类型；**「拆分为年度数据集」**后只显示 `y/x1…xn`，常量列复制到每年。**缓存管理**：删除原始表会级联清除其拆分数据集/统计/探测结果，另有「清空全部缓存」一键重置（前后端同步） |
